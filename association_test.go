...
func OldBelongsTo(t *testing.T) {
	...
	if err := OLDDB.Save(&post).Error; err != nil {
	...
	OLDDB.Model(&post).Association("Category").Append(&category2)
	...
	OLDDB.Model(&post).Related(&category21)
	...
	OLDDB.Model(&post).Association("Category").Replace(&category3)
	...
	OLDDB.Model(&post).Related(&category31)
	...
	// Delete
	OLDDB.Model(&post).Association("Category").Delete(&category2)
	if OLDDB.Model(&post).Related(&Category{}).RecordNotFound() {
	...
	OLDDB.Model(&post).Association("Category").Delete(&category3)
	...
	OLDDB.Model(&post).Related(&category41)
	if category41.Name != "" {
		t.Errorf("Category should be deleted with Delete")
	}

	if count := OLDDB.Model(&post).Association("Category").Count(); count != 0 {
		t.Errorf("Post's category count should be 0 after Delete, but got %v", count)
	}

	// Clear
	OLDDB.Model(&post).Association("Category").Append(&Category{
		Name: "Category 2",
	})
	...
	OLDDB.Model(&post).Association("Category").Clear()
	...
	OLDDB.Model(&post).Association("Category").Append(&category6)

	if count := OLDDB.Model(&post).Association("Category").Count(); count != 1 {
		t.Errorf("Post's category count should be 1 after Append, but got %v", count)
	}

	OLDDB.Delete(&category6)

	if count := OLDDB.Model(&post).Association("Category").Count(); count != 0 {
		t.Errorf("Post's category count should be 0 after the category has been deleted, but got %v", count)
	}

	if err := OLDDB.Model(&post).Association("Category").Find(&Category{}).Error; err == nil {
		t.Errorf("Post's category is not findable after Delete")
	}

	if count := OLDDB.Unscoped().Model(&post).Association("Category").Count(); count != 1 {
		t.Errorf("Post's category count should be 1 when query with Unscoped, but got %v", count)
	}

	if err := OLDDB.Unscoped().Model(&post).Association("Category").Find(&Category{}).Error; err != nil {
		t.Errorf("Post's category should be findable when query with Unscoped, got %v", err)
	}
}

func OldBelongsToOverrideForeignKey1(t *testing.T) {
	type Profile struct {
		oldGorm.Model
		Name string
	}
	...
	if relation, ok := OLDDB.NewScope(&User{}).FieldByName("Profile"); ok {
...
func OldBelongsToOverrideForeignKey2(t *testing.T) {
	type Profile struct {
		oldGorm.Model
		Refer string
		Name  string
...
func OldHasOne(t *testing.T) {
	user := User{
		Name:       "has one",
		CreditCard: CreditCard{Number: "411111111111"},
	}

	if err := OLDDB.Save(&user).Error; err != nil {
	...
	OLDDB.Model(&user).Related(&creditCard1)
	...
	OLDDB.Model(&user).Association("CreditCard").Find(&creditCard11)

	if creditCard11.Number != "411111111111" {
		t.Errorf("Query has one relations with Related")
	}

	if OLDDB.Model(&user).Association("CreditCard").Count() != 1 {
	...
	OLDDB.Model(&user).Association("CreditCard").Append(&creditcard2)
	...
	OLDDB.Model(&user).Related(&creditcard21)
	...
	OLDDB.Model(&user).Association("CreditCard").Replace(&creditcard3)
	...
	OLDDB.Model(&user).Related(&creditcard31)
	...
	// Delete
	OLDDB.Model(&user).Association("CreditCard").Delete(&creditcard2)
	var creditcard4 CreditCard
	OLDDB.Model(&user).Related(&creditcard4)
	...
	OLDDB.Model(&user).Association("CreditCard").Delete(&creditcard3)
	if !OLDDB.Model(&user).Related(&CreditCard{}).RecordNotFound() {
		t.Errorf("Should delete credit card with Delete")
	}

	if OLDDB.Model(&user).Association("CreditCard").Count() != 0 {
	...
	OLDDB.Model(&user).Association("CreditCard").Append(&creditcard5)
	...
	OLDDB.Model(&user).Association("CreditCard").Clear()
	if !OLDDB.Model(&user).Related(&CreditCard{}).RecordNotFound() {
	...
	OLDDB.Model(&user).Association("CreditCard").Append(&creditcard6)

	if count := OLDDB.Model(&user).Association("CreditCard").Count(); count != 1 {
		t.Errorf("User's credit card count should be 1 after Append, but got %v", count)
	}

	OLDDB.Delete(&creditcard6)

	if count := OLDDB.Model(&user).Association("CreditCard").Count(); count != 0 {
		t.Errorf("User's credit card count should be 0 after credit card deleted, but got %v", count)
	}

	if err := OLDDB.Model(&user).Association("CreditCard").Find(&CreditCard{}).Error; err == nil {
		t.Errorf("User's creditcard is not findable after Delete")
	}

	if count := OLDDB.Unscoped().Model(&user).Association("CreditCard").Count(); count != 1 {
		t.Errorf("User's credit card count should be 1 when query with Unscoped, but got %v", count)
	}

	if err := OLDDB.Unscoped().Model(&user).Association("CreditCard").Find(&CreditCard{}).Error; err != nil {
		t.Errorf("User's creditcard should be findable when query with Unscoped, got %v", err)
	}
}

func OldHasOneOverrideForeignKey1(t *testing.T) {
	type Profile struct {
		oldGorm.Model
		Name      string
		UserRefer uint
...
func OldHasOneOverrideForeignKey2(t *testing.T) {
	type Profile struct {
		oldGorm.Model
		Name   string
		UserID uint
...
func OldHasMany(t *testing.T) {
	...
	// Query
	if OLDDB.First(&Comment{}, "content = ?", "Comment 1").Error != nil {
		t.Errorf("Comment 1 should be saved")
	}

	var comments1 []Comment
	OLDDB.Model(&post).Association("Comments").Find(&comments1)
	...
	OLDDB.Model(&post).Association("Comments").Append(&comment6)

	if count := OLDDB.Model(&post).Association("Comments").Count(); count != 1 {
		t.Errorf("post's comments count should be 1 after Append, but got %v", count)
	}

	OLDDB.Delete(&comment6)

	if count := OLDDB.Model(&post).Association("Comments").Count(); count != 0 {
		t.Errorf("post's comments count should be 0 after comment been deleted, but got %v", count)
	}

	var comments6 []Comment
	if OLDDB.Model(&post).Association("Comments").Find(&comments6); len(comments6) != 0 {
		t.Errorf("post's comments count should be 0 when find with Find, but got %v", len(comments6))
	}

	if count := OLDDB.Unscoped().Model(&post).Association("Comments").Count(); count != 1 {
		t.Errorf("post's comments count should be 1 when query with Unscoped, but got %v", count)
	}

	var comments61 []Comment
	if OLDDB.Unscoped().Model(&post).Association("Comments").Find(&comments61); len(comments61) != 1 {
		t.Errorf("post's comments count should be 1 when query with Unscoped, but got %v", len(comments61))
	}
}

func OldHasManyOverrideForeignKey1(t *testing.T) {
	type Profile struct {
		oldGorm.Model
		Name      string
		UserRefer uint
...
func OldHasManyOverrideForeignKey2(t *testing.T) {
	type Profile struct {
		oldGorm.Model
		Name   string
		UserID uint
...
func OldManyToMany(t *testing.T) {
	OLDDB.Raw("delete from languages")
	...
	OLDDB.Model(&user).Related(&newLanguages, "Languages")
	if len(newLanguages) != len([]string{"ZH", "EN"}) {
		t.Errorf("Query many to many relations")
	}

	OLDDB.Model(&user).Association("Languages").Find(&newLanguages)
	if len(newLanguages) != len([]string{"ZH", "EN"}) {
		t.Errorf("Should be able to find many to many relations")
	}

	if OLDDB.Model(&user).Association("Languages").Count() != len([]string{"ZH", "EN"}) {
		t.Errorf("Count should return correct result")
	}

	// Append
	OLDDB.Model(&user).Association("Languages").Append(&Language{Name: "DE"})
	if OLDDB.Where("name = ?", "DE").First(&Language{}).RecordNotFound() {
		t.Errorf("New record should be saved when append")
	}

	languageA := Language{Name: "AA"}
	OLDDB.Save(&languageA)
	OLDDB.Model(&User{Id: user.Id}).Association("Languages").Append(&languageA)

	languageC := Language{Name: "CC"}
	OLDDB.Save(&languageC)
	OLDDB.Model(&user).Association("Languages").Append(&[]Language{{Name: "BB"}, languageC})

	OLDDB.Model(&User{Id: user.Id}).Association("Languages").Append(&[]Language{{Name: "DD"}, {Name: "EE"}})

	totalLanguages := []string{"ZH", "EN", "DE", "AA", "BB", "CC", "DD", "EE"}

	if OLDDB.Model(&user).Association("Languages").Count() != len(totalLanguages) {
		t.Errorf("All appended languages should be saved")
	}

	// Delete
	user.Languages = []Language{}
	OLDDB.Model(&user).Association("Languages").Find(&user.Languages)

	var language Language
	OLDDB.Where("name = ?", "EE").First(&language)
	OLDDB.Model(&user).Association("Languages").Delete(language, &language)

	if OLDDB.Model(&user).Association("Languages").Count() != len(totalLanguages)-1 || len(user.Languages) != len(totalLanguages)-1 {
		t.Errorf("Relations should be deleted with Delete")
	}
	if OLDDB.Where("name = ?", "EE").First(&Language{}).RecordNotFound() {
		t.Errorf("Language EE should not be deleted")
	}

	OLDDB.Where("name IN (?)", []string{"CC", "DD"}).Find(&languages)

	user2 := User{Name: "Many2Many_User2", Languages: languages}
	OLDDB.Save(&user2)

	OLDDB.Model(&user).Association("Languages").Delete(languages, &languages)
	if OLDDB.Model(&user).Association("Languages").Count() != len(totalLanguages)-3 || len(user.Languages) != len(totalLanguages)-3 {
		t.Errorf("Relations should be deleted with Delete")
	}

	if OLDDB.Model(&user2).Association("Languages").Count() == 0 {
		t.Errorf("Other user's relations should not be deleted")
	}

	// Replace
	var languageB Language
	OLDDB.Where("name = ?", "BB").First(&languageB)
	OLDDB.Model(&user).Association("Languages").Replace(languageB)
	if len(user.Languages) != 1 || OLDDB.Model(&user).Association("Languages").Count() != 1 {
		t.Errorf("Relations should be replaced")
	}

	OLDDB.Model(&user).Association("Languages").Replace()
	if len(user.Languages) != 0 || OLDDB.Model(&user).Association("Languages").Count() != 0 {
		t.Errorf("Relations should be replaced with empty")
	}

	OLDDB.Model(&user).Association("Languages").Replace(&[]Language{{Name: "FF"}, {Name: "JJ"}})
	if len(user.Languages) != 2 || OLDDB.Model(&user).Association("Languages").Count() != len([]string{"FF", "JJ"}) {
		t.Errorf("Relations should be replaced")
	}

	// Clear
	OLDDB.Model(&user).Association("Languages").Clear()
	if len(user.Languages) != 0 || OLDDB.Model(&user).Association("Languages").Count() != 0 {
...
func OldRelated(t *testing.T) {
	...
	var emails []Email
	OLDDB.Model(&user).Related(&emails)
	if len(emails) != 2 {
		t.Errorf("Should have two emails")
	}

	var emails2 []Email
	OLDDB.Model(&user).Where("email = ?", "jinzhu@example.com").Related(&emails2)
	if len(emails2) != 1 {
		t.Errorf("Should have two emails")
	}

	var emails3 []*Email
	OLDDB.Model(&user).Related(&emails3)
	if len(emails3) != 2 {
		t.Errorf("Should have two emails")
	}

	var user1 User
	OLDDB.Model(&user).Related(&user1.Emails)
	if len(user1.Emails) != 2 {
		t.Errorf("Should have only one email match related condition")
	}

	var address1 Address
	OLDDB.Model(&user).Related(&address1, "BillingAddressId")
	if address1.Address1 != "Billing Address - Address 1" {
		t.Errorf("Should get billing address from user correctly")
	}

	user1 = User{}
	OLDDB.Model(&address1).Related(&user1, "BillingAddressId")
	if OLDDB.NewRecord(user1) {
		t.Errorf("Should get user from address correctly")
	}

	var user2 User
	OLDDB.Model(&emails[0]).Related(&user2)
	...
	var user3 User
	OLDDB.First(&creditcard, "number = ?", "1234567890")
	OLDDB.Model(&creditcard).Related(&user3)
	if user3.Id != user.Id || user3.Name != user.Name {
		t.Errorf("Should get user from credit card correctly")
	}

	if !OLDDB.Model(&CreditCard{}).Related(&User{}).RecordNotFound() {
		t.Errorf("RecordNotFound for Related")
	}

	var company Company
	if OLDDB.Model(&user).Related(&company, "Company").RecordNotFound() || company.Name != "company1" {
		t.Errorf("RecordNotFound for Related")
	}
}

func OldForeignKey(t *testing.T) {
	for _, structField := range OLDDB.NewScope(&User{}).GetStructFields() {
	...
	for _, structField := range OLDDB.NewScope(&Email{}).GetStructFields() {
	...
	for _, structField := range OLDDB.NewScope(&Post{}).GetStructFields() {
	...
	for _, structField := range OLDDB.NewScope(&Comment{}).GetStructFields() {
...
func testForeignKey(t *testing.T, source interface{}, sourceFieldName string, target interface{}, targetFieldName string) {
	...
	targetScope := OLDDB.NewScope(target)
	targetTableName := targetScope.TableName()
	modelScope := OLDDB.NewScope(source)
...
func OldSkipSaveAssociation(t *testing.T) {
	type Company struct {
		oldGorm.Model
		Name string
	}
	...
	OLDDB.AutoMigrate(&Company{}, &User{})

	OLDDB.Save(&User{Name: "jinzhu", Company: Company{Name: "skip_save_association"}})

	if !OLDDB.Where("name = ?", "skip_save_association").First(&Company{}).RecordNotFound() {
