...
func OldUpdate(t *testing.T) {
	product1 := Product{Code: "product1code"}
	product2 := Product{Code: "product2code"}

	OLDDB.Save(&product1).Save(&product2).Update("code", "product2newcode")

	if product2.Code != "product2newcode" {
		t.Errorf("Record should be updated")
	}

	OLDDB.First(&product1, product1.Id)
	OLDDB.First(&product2, product2.Id)
	updatedAt1 := product1.UpdatedAt

	if OLDDB.First(&Product{}, "code = ?", product1.Code).RecordNotFound() {
		t.Errorf("Product1 should not be updated")
	}

	if !OLDDB.First(&Product{}, "code = ?", "product2code").RecordNotFound() {
		t.Errorf("Product2's code should be updated")
	}

	if OLDDB.First(&Product{}, "code = ?", "product2newcode").RecordNotFound() {
		t.Errorf("Product2's code should be updated")
	}

	OLDDB.Table("products").Where("code in (?)", []string{"product1code"}).Update("code", "product1newcode")

	var product4 Product
	OLDDB.First(&product4, product1.Id)
	...
	if !OLDDB.First(&Product{}, "code = 'product1code'").RecordNotFound() {
		t.Errorf("Product1's code should be updated")
	}

	if OLDDB.First(&Product{}, "code = 'product1newcode'").RecordNotFound() {
		t.Errorf("Product should not be changed to 789")
	}

	if OLDDB.Model(product2).Update("CreatedAt", time.Now().Add(time.Hour)).Error != nil {
		t.Error("No error should raise when update with CamelCase")
	}

	if OLDDB.Model(&product2).UpdateColumn("CreatedAt", time.Now().Add(time.Hour)).Error != nil {
		t.Error("No error should raise when update_column with CamelCase")
	}

	var products []Product
	OLDDB.Find(&products)
	if count := OLDDB.Model(Product{}).Update("CreatedAt", time.Now().Add(2*time.Hour)).RowsAffected; count != int64(len(products)) {
		t.Error("RowsAffected should be correct when do batch update")
	}

	OLDDB.First(&product4, product4.Id)
	updatedAt4 := product4.UpdatedAt
	OLDDB.Model(&product4).Update("price", gorm.Expr("price + ? - ?", 100, 50))
	var product5 Product
	OLDDB.First(&product5, product4.Id)
...
func OldUpdateWithNoStdPrimaryKeyAndDefaultValues(t *testing.T) {
	animal := Animal{Name: "Ferdinand"}
	OLDDB.Save(&animal)
	updatedAt1 := animal.UpdatedAt

	OLDDB.Save(&animal).Update("name", "Francis")
	...
	var animals []Animal
	OLDDB.Find(&animals)
	if count := OLDDB.Model(Animal{}).Update("CreatedAt", time.Now().Add(2*time.Hour)).RowsAffected; count != int64(len(animals)) {
		t.Error("RowsAffected should be correct when do batch update")
	}

	animal = Animal{From: "somewhere"}                 // No name fields, should be filled with the default value (galeone)
	OLDDB.Save(&animal).Update("From", "a nice place") // The name field shoul be untouched
	OLDDB.First(&animal, animal.Counter)
...
func OldUpdates(t *testing.T) {
	product1 := Product{Code: "product1code", Price: 10}
	product2 := Product{Code: "product2code", Price: 10}
	OLDDB.Save(&product1).Save(&product2)
	OLDDB.Model(&product1).Updates(map[string]interface{}{"code": "product1newcode", "price": 100})
	...
	if OLDDB.First(&Product{}, "code = ? and price = ?", product2.Code, product2.Price).RecordNotFound() {
	...
	OLDDB.Table("products").Where("code in (?)", []string{"product2code"}).Updates(Product{Code: "product2newcode"})
	if !OLDDB.First(&Product{}, "code = 'product2code'").RecordNotFound() {
		t.Errorf("Product2's code should be updated")
	}

	var product4 Product
	OLDDB.First(&product4, product2.Id)
	...
	updatedAt4 := product4.UpdatedAt
	OLDDB.Model(&product4).Updates(map[string]interface{}{"price": gorm.Expr("price + ?", 100)})
	var product5 Product
	OLDDB.First(&product5, product4.Id)
...
func OldUpdateColumn(t *testing.T) {
	product1 := Product{Code: "product1code", Price: 10}
	product2 := Product{Code: "product2code", Price: 20}
	OLDDB.Save(&product1).Save(&product2).UpdateColumn(map[string]interface{}{"code": "product2newcode", "price": 100})
	if product2.Code != "product2newcode" || product2.Price != 100 {
		t.Errorf("product 2 should be updated with update column")
	}

	var product3 Product
	OLDDB.First(&product3, product1.Id)
	...
	OLDDB.First(&product2, product2.Id)
	updatedAt2 := product2.UpdatedAt
	OLDDB.Model(product2).UpdateColumn("code", "update_column_new")
	var product4 Product
	OLDDB.First(&product4, product2.Id)
	if updatedAt2.Format(time.RFC3339Nano) != product4.UpdatedAt.Format(time.RFC3339Nano) {
	...
	OLDDB.Model(&product4).UpdateColumn("price", gorm.Expr("price + 100 - 50"))
	var product5 Product
	OLDDB.First(&product5, product4.Id)
	if product5.Price != product4.Price+100-50 {
...
func OldSelectWithUpdate(t *testing.T) {
	user := getPreparedUser("select_user", "select_with_update")
	OLDDB.Create(user)

	var reloadUser User
	OLDDB.First(&reloadUser, user.Id)
	...
	OLDDB.Select("Name", "BillingAddress", "CreditCard", "Company", "Emails").Save(&reloadUser)
...
func OldSelectWithUpdateWithMap(t *testing.T) {
	...
	var reloadUser User
	OLDDB.First(&reloadUser, user.Id)
	OLDDB.Model(&reloadUser).Select("Name", "BillingAddress", "CreditCard", "Company", "Emails").Update(updateValues)
...
func OldOmitWithUpdate(t *testing.T) {
	...
	OLDDB.Omit("Name", "BillingAddress", "CreditCard", "Company", "Emails").Save(&reloadUser)
...
func OldOmitWithUpdateWithMap(t *testing.T) {
	user := getPreparedUser("select_user", "select_with_update_map")
	OLDDB.Create(user)
	...
	var reloadUser User
	OLDDB.First(&reloadUser, user.Id)
	OLDDB.Model(&reloadUser).Omit("Name", "BillingAddress", "CreditCard", "Company", "Emails").Update(updateValues)
...
func OldSelectWithUpdateColumn(t *testing.T) {
	user := getPreparedUser("select_user", "select_with_update_map")
	OLDDB.Create(user)
	...
	var reloadUser User
	OLDDB.First(&reloadUser, user.Id)
	OLDDB.Model(&reloadUser).Select("Name").UpdateColumn(updateValues)

	var queryUser User
	OLDDB.First(&queryUser, user.Id)
...
func OldOmitWithUpdateColumn(t *testing.T) {
	user := getPreparedUser("select_user", "select_with_update_map")
	OLDDB.Create(user)
	...
	var reloadUser User
	OLDDB.First(&reloadUser, user.Id)
	OLDDB.Model(&reloadUser).Omit("Name").UpdateColumn(updateValues)
...
func OldUpdateColumnsSkipsAssociations(t *testing.T) {
	...
	db := OLDDB.Model(user).UpdateColumns(User{Age: newAge})
	if db.RowsAffected != 1 {
		t.Errorf("Expected RowsAffected=1 but instead RowsAffected=%v", OLDDB.RowsAffected)
	}

	// Verify that Age now=`newAge`.
	freshUser := &User{Id: user.Id}
	OLDDB.First(freshUser)
	...
	OLDDB.First(&freshUser.BillingAddress, freshUser.BillingAddressID)
...
func OldUpdatesWithBlankValues(t *testing.T) {
	product := Product{Code: "product1", Price: 10}
	OLDDB.Save(&product)

	OLDDB.Model(&Product{Id: product.Id}).Updates(&Product{Price: 100})

	var product1 Product
	OLDDB.First(&product1, product.Id)
...
func OldUpdatesTableWithIgnoredValues(t *testing.T) {
	elem := ElementWithIgnoredField{Value: "foo", IgnoredField: 10}
	OLDDB.Save(&elem)

	OLDDB.Table(elem.TableName()).
		Where("id = ?", elem.Id).
	...
	err := OLDDB.First(&elem1, elem.Id).Error
	if err != nil {
...
func OldUpdateDecodeVirtualAttributes(t *testing.T) {
	var user = User{
		Name:     "jinzhu",
		IgnoreMe: 88,
	}
	...
	OLDDB.Model(&user).Updates(User{Name: "jinzhu2", IgnoreMe: 100})
