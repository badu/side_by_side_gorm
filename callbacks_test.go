...
func OldRunCallbacks(t *testing.T) {
	...
	OLDDB.Where("Code = ?", "unique_code").First(&p)
	...
	OLDDB.Find(&products, "code = ?", "unique_code")
	...
	if OLDDB.Where("Code = ?", "unique_code").First(&p).Error == nil {
		t.Errorf("Can't find a deleted record")
	}
}

func OldCallbacksWithErrors(t *testing.T) {
	p := Product{Code: "Invalid", Price: 100}
	if OLDDB.Save(&p).Error == nil {
		t.Errorf("An error from before create callbacks happened when create with invalid value")
	}

	if OLDDB.Where("code = ?", "Invalid").First(&Product{}).Error == nil {
		t.Errorf("Should not save record that have errors")
	}

	if OLDDB.Save(&Product{Code: "dont_save", Price: 100}).Error == nil {
