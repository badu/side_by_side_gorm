...
func OldScannableSlices(t *testing.T) {
	if err := OLDDB.AutoMigrate(&RecordWithSlice{}).Error; err != nil {
	...
	if err := OLDDB.Save(&r1).Error; err != nil {
		t.Errorf("Should save record with slice values")
	}

	var r2 RecordWithSlice

	if err := OLDDB.Find(&r2).Error; err != nil {
